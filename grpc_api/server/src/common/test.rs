...
impl TestServer {
    ...
    pub fn new(kind: TestServerKind) -> TestServer
    {
        ...
        let (env_db, env_addr, bin_name) = match kind {
            TestServerKind::Auth => ("DATABASE_URL_AUTH_TEST", "SERVER_ADDRESS_AUTH", "auth_server"),
            TestServerKind::Resource => ("DATABASE_URL_RESOURCE_TEST", "SERVER_ADDRESS_RESOURCE", "resource_server")
        };
    ...
    pub async fn truncate_tables(&self) -> Result<(), Error>
    {
        ...
        let sql = match self.kind {
            ...
            TestServerKind::Resource => "TRUNCATE TABLE \"slice_data_set\", \"slice_data\", \"data_buffer\", \"data\", \"set_map\", \"set_template_map\", \"set\", \"set_template\", \"group_model_map\", \"group_device_map\", \"group_model\", \"group_device\", \"device_config\", \"device\", \"device_type_config\", \"device_type_model\", \"device_type\", \"model_tag_member\", \"model_tag\", \"model_config\", \"model\";"
        };
        sqlx::query(sql)
            .execute(&pool)
            .await?;
    ...
    pub fn start_server(&self)
    {
        ...
        let args: Vec<&str> = if self.secured {
            match &self.api_cred {
                Some((id, pw, addr)) => vec![
                    "run", "-p", "bbthings_grpc_server", "--bin", self.bin_name.as_str(),
            ...
            vec![
                "run", "-p", "bbthings_grpc_server", "--bin", self.bin_name.as_str(), "--", "--db-url", self.db_url.as_str()
            ]
        };
        Command::new("cargo")
            .args(args)
